	/* A simple calculator:
	   Basic functions: 
		Clear 				- clear()
		Clear All 			- clearAllValues()
		Apply negation 		- setInverseSign(), negateNumber()
		Calculate 			- performCalculation(), 
			Add
			Subtract
			Multiple
			Divide
		Set numbers, decimal - setNumbers()
		Set up operators 	 - setOperators()
		Events: Mouse click, keyboard selection on numbers and num lock keys
		JQuery or third party scripts were not needed, all code is in plain JavaScript for the scope of the test.
		'callCalculator' is the main method within which all the functionality is defined. 
		
	   Event handler setup for keyboard events:
				setKeypressEvents()
		
	   Supporting methods to make everything work:
		Check for string is null or empty, for reuse: 
			stringNullOrEmpty()
		Setting event handlers by classes: 	
			getElementsByClassNameBackup()
			setEventHandlerForClass()
	*/
	...
	/***** Event handler setup for calculator ******/			
	/***** Supporter methods for event handler binding/setup 
			Can be placed in common file for reuse *****/
	//	getElementsByClassName works in browsers tested except IE8-
	//	Backup method for IE8- and other non supported browsers!!
	...
	//Set up click events for all elements with a specific className
	...
	/*** Set up  calculator specific event handlers 
		 plain Javascript - without Jquery/other third party scripts 
	****/
	...
	var setKeypressEvents = function(event){ 
		...
		if(console && console.log) { console.log('event.keyCode: '+ event.keyCode + ', event.code: '+event.code);  }
			/* Get the keycode by looking at my local debugger for this page, and looking at 'event' in the this method:
			  Reference copied:
				...
				* : 42
				...
				% : 37
				= : 61, Enter key is 13	*/	
			//	Don't see an event for 'backspace' or clear one character!
						
			// In google chrome, event.keyCode and event.which have same value. 
			// To Do: Check in IE8- but atleast have 'which' as a backup?
			...
			if(keycode === 46) {
				...
				decimalEl[0].click(); 
				//setEventHandlerForClass('decimal', setNumbers, 'click'); 
			}
	...
	//When user clicks on 'number', 'operator' set up some callbacks:
	...
	if(!document.addEventListener){	//IE8-
			...
			//Event handlers: Works in most new browsers
			...
			//If user presser enter, or clicks on 'calculate', we perform some extra validations to ensure everything works:
			document.getElementById('calculate').addEventListener('click', function(){
				finalCalculation(); 
			});						
	 }		
  }(); //End of callCalculator
